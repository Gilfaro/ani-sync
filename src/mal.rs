...
impl MalClient {
    ...
    /// Fetches the anime list for a user.
    ///
    /// # Errors
    ///
    /// Returns an error if the request fails or the response cannot be parsed.
    pub async fn get_user_anime_list(&self, user_name: &str) -> Result<Vec<MalEntry>> {
        ...
        while let Some(url) = next_url {
            ...
            let page: MalListResponse = serde_json::from_str(&text)?;
    ...
    /// Fetches the manga list for a user.
    ///
    /// # Errors
    ///
    /// Returns an error if the request fails or the response cannot be parsed.
    pub async fn get_user_manga_list(&self, user_name: &str) -> Result<Vec<MalEntry>> {
        let mut all_entries = Vec::new();
